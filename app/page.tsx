...
export default function CountriesApp() {
  const [countryStates, setCountryStates] = useState<Record<string, ViewState>>(
    countries.reduce((acc, country) => ({ ...acc, [country.id]: "flag" }), {}),
  )
  ...
  const handleCardClick = (countryId: string) => {
    setCountryStates((prev) => {
      const currentState = prev[countryId]
...
.library-background::before {
  ...
  right: 0;
  bottom: 0;
  background: rgba(255, 255, 255, 0.65); /* Reducido de 0.85 a 0.65 para menos difuminación */
  backdrop-filter: blur(0.3px); /* Reducido de 1px a 0.3px para más nitidez */
  z-index: 0;
}
...
.hero-section::before {
  ...
  right: 0;
  bottom: 0;
  background: rgba(139, 69, 19, 0.75); /* Overlay dorado/marrón para el libro mágico */
...
.hero-overlay {
  background: transparent; /* Quitar el fondo crema */
  backdrop-filter: none; /* Quitar el blur */
  border-radius: 30px;
  padding: 3rem;
  margin: 2rem;
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-16 lg:py-24">
          <div className="hero-overlay text-center">
            ...
            <div className="flex items-center justify-center gap-4 mb-8">
              ...
              <cite className="literary-body text-lg lg:text-xl font-medium text-white not-italic">
                Caitlin Moran
              </cite>
              <div className="h-1 w-16 bg-amber-700"></div>
            </div>

            <p className="literary-body text-white text-lg mb-8">Autora y periodista británica</p>
